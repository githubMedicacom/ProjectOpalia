...
import { releveMedicalAdded, saveFile, extractionsReleve, cheeckProduit, verifReleve } from "../../Redux/releveMedicalReduce";
...
function ReleveMedicalGro() {
      ...
      dispatch(cheeckProduit(table)).then((rowsdes) => {
        if (rowsdes !== undefined && rowsdes.payload.length > 0) {
          ...
          for (var i = 0; i < prod.length; i++) {
            ...
            var vente = 0;
            if (table[i].stock)
              if (typeof table[i].stock == "number" || !isNaN(table[i].stock.toString().replaceAll(" ",""))) { stock = table[i].stock.toString().replaceAll(" ","") } else  {stock = 0}
              /* else { if(table[i].stock !== "" && table[i].stock !== " ") stock = table[i].stock.replace(' ', '') } */
            if (table[i].vente)
              if (typeof table[i].vente== "number" || !isNaN(table[i].vente.toString().replaceAll(" ","") )) { vente = table[i].vente.toString().replaceAll(" ","") }
              else {vente = 0}
            if (prod[i] !== null && typeof prod[i] !== "undefined" && prod[i] !== "undefined") {
              arrayBody.push({
                ...
                code: 
                table[i].code.toString().toLowerCase() !== "nan" &&
                table[i].code.toString().toLowerCase() !== "not found"
                  ?table[i].code.toString():"",
                vente: vente.toString().toLowerCase() !== "not found" && vente.toString().toLowerCase() !== "nan" ?vente.toString().replaceAll(" ",""):0,
                stock: stock.toString().toLowerCase() !== "not found" && stock.toString().toLowerCase() !== "nan" ?stock.toString().replaceAll(" ",""):0,
                mois: table[i].month,
                annee: table[i].annee,
                na: 'N/A'
              });
  ...
  const saveTable = useCallback(
    async (entities, uploadFile, fourSelected, annee, mois,type,typeGro) => {
     ...
     { entities.forEach((data) => {
      ...
      if (verif === false) {
       ...
       dispatch(releveMedicalAdded({type:type.value,typeGro:typeGro.value, annee: annee.value, mois: mois.value, file: filename, namefile: uploadFile.name, ligneReleve: entities, four: fourSelected.value })).then((e) => {
              if (e.payload === true) {
                ...
                <Row className="hidden" id="table-BL">
                  ...
                  <Col md="12">
                    ...
                    <Button
                      ...
                      variant="success"
                      onClick={() =>
                        saveTable(entities, uploadFile, fourSelected, annee, mois,type,typeGro)
                      }
                    >
                      <span className="btn-label">
...
export default ReleveMedicalGro;
