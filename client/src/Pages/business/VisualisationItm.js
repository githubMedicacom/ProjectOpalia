...
import { allItm, deleteItm } from "../../Redux/itmReduce";
...
function VisualisationItm() {
  ...
  const confirmMessage = (bl, idRow) => {
    setAlert(
      <SweetAlert
        ...
        title={"Étes Vous sure de supprimer ce itm?"}
        onConfirm={() => {
          deleteItms(bl.id, idRow);
        }}
        onCancel={() => hideAlert()}
  ...
  function deleteItms(id, idRow) {
    var list = data;
    list.splice(idRow, 1);
    setData(list);
    console.log(id)
    dispatch(deleteItm({ id }));
  ...
  function getFile(idBl) {
    navigate("/telechargerFichierItm/" + idBl);
  }
  ...
  return (
    <>
      ...
      <Container fluid>
        ...
        <Row>
          <Col md="12">
            <h4 className="title">Liste des itms</h4>
            <Card>
                ...
                <ReactTable
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      accessor: "type",
                      Cell: ({ cell }) =>
                      parseInt(cell.row.values.type)== 1 ?'Current':"Fin mois"
                                        },
                    {
...
export default VisualisationItm;
