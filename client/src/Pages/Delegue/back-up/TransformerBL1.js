...
function TransformerBL() {
    ...
    async (table, objFour, tva, objProduit) => {
      ...
      dispatch(cheeckProduit(table)).then((rowsdes) => {
        ...
        for (var i = 0; i < prod.length; i++) {
          ...
          var mnt = prix * qte;
          var ttc = mnt;
          /* if (objFour && objFour.tva === 1) {
            ttc = parseFloat(mnt) * (1 + tva.tva / 100);
          } */
          if (prod[i] !== null) {
  ...
  const showTable = useCallback(
    async (file, objFour, tva, objProduit) => {
      var element = document.getElementById("table-BL");
      var element1 = document.getElementById("table-BL-header");
      element.classList.add("hidden");
      element1.classList.add("hidden");
      ...
      if (
        ...
        if (entities.length > 0) {
          entities.forEach((data) => {
            ...
            if (data.Montant === "" || parseInt(data.Montant) === 0) verif = false;
            ...
            <Card className="strpied-tabled-with-hover">
              ...
              <Card.Body>
                ...
                <Button
                  ...
                  onClick={(v) => {
                    if (traitment === false)
                      showTable(uploadFile, objFour, tva, objProduit);
                    else if (cropData !== "#") {
