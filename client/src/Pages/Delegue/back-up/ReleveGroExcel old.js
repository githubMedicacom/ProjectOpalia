...
import { releveAddedExcel, saveFile, extractionsReleve, cheeckProduit, verifReleve } from "../../Redux/releveReduce";
...
const fileHandler = (e) => {
  ...
  ExcelRenderer(fileObj, (err, resp) => {
    if (err) {
      ...
      resp.rows.slice(1).map((row, index) => {
        let code="";  
      if(row[0] !=undefined) code=typeof row[0]=="string"?row[0].replaceAll(" ","") :row[0].toString()
        ...
        if(rowsdes !== undefined && rowsdes.payload.length >0 ){
          ...
          for (var i = 0; i < prod.length; i++) {  
            if (prod[i] !== null && typeof prod[i] !== "undefined" && prod[i] !== "undefined"  ) {
              ...
              if(table[i].code!="") produitexit.push('Code produit ' + table[i].code + " \n\n/ \n\n");
                arrayBody.push({
                  idProduit: null,
  ...
  const showTable = useCallback(
    async (file, type, mois, fourSelected, annee) => {
      if (file && (type !== null && type.value !== "") && (mois !== null && mois.value !== "") && (annee !== null && annee.value !== "") && (fourSelected !== null && fourSelected.label !== "")) {
  ...
  const saveTable = useCallback(
    async (entities, uploadFile, fourSelected, annee, mois) => {
      ...
      if (verif === false) {
        ...
        if (fourSelected !== null) {
          dispatch(saveFile(dataArray)).then((e) => {
            ...
            dispatch(releveAddedExcel({ annee: annee.value, mois: mois.value, file: filename, namefile: uploadFile.name, ligneReleve: entities, four: fourSelected.value })).then((e) => {
              if (e.payload === true) {
              ...
              <Card.Body>
                <div className="text-center">
                  ...
                  <table className="table responsive-BL ">
                    <thead className="rt-thead -header"><tr><td>Code</td><td>Designation</td><td>Stock</td><td>Vente</td><td>Date limite</td></tr></thead>
                  </table>
                  <br />
                  <Button
                ...
                <Row className="hidden" id="table-BL">
                  ...
                  <Col md="12">
                    ...
                    <Button
                      ...
                      variant="success"
                      onClick={() =>
                        saveTable(entities, uploadFile, fourSelected, annee, mois)
                      }
                    >
                      <span className="btn-label">
