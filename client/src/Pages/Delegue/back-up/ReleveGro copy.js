      ...
      dispatch(cheeckProduit(table)).then((rowsdes) => {
        if(rowsdes !== undefined && rowsdes.payload.length >0 ){
          ...
          for (var i = 0; i < prod.length; i++) {
            var mesure =0;
            if(typeof table[i].Mesure=="number"){mesure=table[i].Mesure}
            else{mesure=table[i].Mesure.replace(' ','')}
            if (prod[i] !== null && typeof prod[i] !== "undefined" && prod[i] !== "undefined"  ) {
              arrayBody.push({
                ...
                code: table[i].code,
                type: table[i].type,
                mesure: (table[i].Mesure != "" && table[i].Mesure != " ") ? mesure : 0,
                mois: table[i].month,
                annee: table[i].annee,
                na: 'N/A'
              });
      ...
      if (verif === false) {
        ...
        if (fourSelected !== null) {
          dispatch(saveFile(dataArray)).then((e) => {
            ...
            dispatch(releveAdded({ annee: annee.value, mois: mois.value, file: filename, namefile: uploadFile.name, ligneReleve: entities, four: fourSelected.value })).then((e) => {
              if (e.payload === true) {
                      ...
                      columns={[
                            ...
                            <div>
                              <Form.Group>
                                <Form.Control
                                  ...
                                  onBlur={(value) => {
                                    var e = entities;
                                    e[cell.row.id].mesure = value.target.value;
                                    setTable(e, 1);
                                  }}
                                ></Form.Control>
                              </Form.Group>
                        ...
                        },
                        {
                          ...
                          (<div className="table-bl table-bl135">
                            {entities[cell.row.id].type !== null ? (
                              parseInt(cell.row.values.type) === 1 ? "Stock" : "Vente") :
                              <Select
                                className="react-select primary "
                                classNamePrefix="react-select"
                                name="Type"
