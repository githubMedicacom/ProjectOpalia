...
import {
  getFileReleveVente,
  getDetailReleveVente,
} from "../../Redux/releveVenteReduce";
import SweetAlert from "react-bootstrap-sweetalert";
function TelechargerFichierReleveVente() {
  document.title = "Détails relevé vente";
  ...
  const detailBl = useCallback(async () => {
    var det = await dispatch(getDetailReleveVente(idBl));
    var data = await det.payload;
    setAlert(
      <SweetAlert
        customClass="pop-up-bl"
        style={{ display: "block", marginTop: "-100px" }}
        title={"Détail Relevé Vente"}
        onConfirm={() => hideAlert()}
            ...
            {data.map((e) => {
              ...
              return (
                <tr key={"ligne-" + e.id}>
                  ...
                  <td>{type}</td>
                  <td>
                    {e.date == null || e.date == "0000-00-00" ? "N/A" : e.date}
                  </td>
                </tr>
              );
            })}
  ...
  const getFile = React.useCallback(
    async (file) => {
      ...
      if (fileLocal !== "null") {
        ...
        setExt(ext1);
        dispatch(getFileReleveVente(idBl)).then(async (e1) => {
          var ff = null;
            ...
            <Row>
              <Col md="12">
                <Button
                  ...
                  variant="info"
                  onClick={() => {
                    navigate("/VisualisationReleveVente");
                  }}
                >
...
export default TelechargerFichierReleveVente;
