...
function ListeProduits() {
  ...
  const listeProduits = useCallback(
    async (trimestre, anneeLocal, cmdeBls) => {
      ...
      res.forEach((element) => {
        ...
        for (const key in cmdeBls) {
          const element = cmdeBls[key];
          if (key == val.idbL) {
            val.dateValidation = element;
          }
        }

        console.log(val.dateValidation)
  ...
  const exporter = useCallback(
    async (trimestre, anneeLocal) => {
      dispatch(exportBl()).then((e) => {
        ...
        hideAlert();
        listeProduits(trimestre, anneeLocal, cmdeBls);
      });
    },
