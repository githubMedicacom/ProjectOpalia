...
/**
 * function hedhi bech najbdo pharmacie illi 3adaw commande w nejbdohom hasb user connecter 
 * **/
...
router.get("/totalCaByAction/:idAction", auth, async (req, res) => {
  ...
  if (findBl.dataValues.id_bls) {
    ...
    var mntTotal = await ligneBl.sum("montant_ttc", {
      where: {
...
//Dasbord action Objectif/Réalisé
        ...
        if (findGroup.length == 1 && findGroup[0].dataValues.id_pack == 0) {
          ...
          var mnt_produit = await ligneBl.findAll({
            attributes: [
              ...
              [sequelize.fn("sum", sequelize.col("montant_ttc")), "montant"],
            ],
            include: [
              {
                model: bl,
...
router.get("/getColorClient/:idAction/:idClient", auth, async (req, res) => {
  try {
    ...
    var test = true;
    var totalPack = ligneBl.sum("montant_ttc", {
      include: {
        model: bl,
        as: "bls",
...
router.get("/getTypePayments/:idCmd/:mnt", auth, async (req, res) => {
  ...
  return res
    .status(200)
    .send({
      action_amm: action_amm,
      action_amc: action_amc,
      actions: findCmd.commandes.actions,
    });
});
...
router.get("/getBlByCmd/:idCmd", auth, async (req, res) => {
  var idCmd = req.params.idCmd;
  var findCmd = await commande_bl.findAll({
    include: [
      {
        model: bl,
        as: "bls",
