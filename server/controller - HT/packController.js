...
router.post("/getPackByProduits", auth, async (req, res) => {
  ...
  for (const key in arrayFinal) {
    if (idClient !== null) {
      ...
      /* var final = await ligneBl.findOne({
        attributes: [[sequelize.fn("sum", sequelize.col("montant")), "mnt"]],
        where: {
          [Op.and]: [
            { id_pack: { [Op.ne]: 0 }, idproduit: arrayFinal[key].idProduit },
          ],
        },
        include: [
          {
            model: bl,
            as: "bls",
            where: { client: idClient, id_action: id_action, iduser: idUser },
          },
        ],
      });
      var mntTotal = final.dataValues.mnt ? final.dataValues.mnt.toFixed(3) : 0; */
      if (findProduit) {
        /* if (
          findProduit.dataValues.montant > parseFloat(mntTotal) ||
          parseFloat(mntTotal) == 0
        ) { */
        ...
        /* } else {
          ...
          arrayRemove[findProduit.dataValues.packId] = false;
          test = true;
          packSelected.push({
            value: 0,
            label: "Commande spontanée",
