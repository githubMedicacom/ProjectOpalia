  ...
  if (id == 0) {
    actionComercials
      ...
      .then((r) => {
        ...
        /* var arrayU = [];
        users.forEach(val=>{
          val.id_action = r.dataValues.id;
          arrayU.push(val); 
        }) 
        actionDelegue.bulkCreate(arrayU).then(() => {}); */
...
function packFromBl(action) {
  return new Promise((resolve, reject) => {
    ...
    var sql = `SELECT sum(li.montant) as som FROM bls b 
...
function packFromBlBydelegue(action) {
  return new Promise((resolve, reject) => {
    ...
    var sql = `SELECT sum(li.montant) as som,u.nomU,u.prenomU
    FROM bls b 
    left join lignebls li on li.idbielle = b.id
